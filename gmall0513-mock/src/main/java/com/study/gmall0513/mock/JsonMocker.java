...
import com.study.gmall0513.mock.util.*;
...
public class JsonMocker {
    ...
    String initStartupLog() {  //启动日志
     ...
     `area` string COMMENT '城市' */
        ...
        String uid = "uid" + RandomNum.getRandInt(1, 500);  //用过id
    ...
    public static void genLog() {   //生成日志
        JsonMocker jsonMocker = new JsonMocker();
        jsonMocker.startupNum = 1000000;
        for (int i = 0; i < jsonMocker.startupNum; i++) {
            String startupLog = jsonMocker.initStartupLog();  //发送
            jsonMocker.sendLog(startupLog);
            while (!jsonMocker.isQuitGroup.getRandomOpt().getValue()) {  //事件日志
